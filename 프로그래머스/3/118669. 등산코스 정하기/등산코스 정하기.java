...
class Solution {
    ...
    public int[] solution(int n, int[][] paths, int[] gates, int[] summits) {
        for (int i = 0; i < graphs.length; i++) graphs[i] = new ArrayList<Node>();
        for (int i = 0; i <= n; i++) visited[i] = 1000000000;
        for (int i = 0; i < summits.length; i++) isSummits[summits[i]] = true;
        for (int[] path: paths) {
            int x = path[0];
            int y = path[1];
            int z = path[2];
