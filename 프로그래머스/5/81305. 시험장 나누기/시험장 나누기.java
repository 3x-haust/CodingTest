...
class Solution {
    ...
    private int dfs(int[] num, int cur, int max){
        ...
        if(tree[cur].left != -1) lv = dfs(num, tree[cur].left, max);
        if(tree[cur].right != -1) rv = dfs(num, tree[cur].right, max);
        
        // 자식 노드들과 자신의 값의 합이 max보다 크면 자르기
        if(num[cur] + lv + rv <= max) return num[cur] + lv + rv;
        
        // 자신 노드들 중 제일 작은 값과 자신의 값의 합이 max보다 작거나 같으면 한개만 자르기
        ...
        // 둘 다 잘라야하는 경우 
        // 맨 밑단 노드 값이 max보다 작은 경우는 없다
        // low가 제일 큰 노드를 기준으로 잡히기 때문에 무조건 첫번째 if에서 통과된다
        size += 2;
        return num[cur];
    }
