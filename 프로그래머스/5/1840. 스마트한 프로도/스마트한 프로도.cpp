...
struct edge {
    ...
    bool isadjacent(edge* e2) {
        if(node1 == e2 -> node1 || node2 == e2 -> node1 || node1 == e2 -> node2 || node2 == e2 -> node2) return true;
        else return false;
    }
};
struct node {
...
struct edge_set {
    ...
    bool iscycle() {
        ...
        if(front -> isadjacent(back)) return true;
        else return false;
    }
    ...
    vector<vector<int>> process() {
        ...
        for(int i = 0; i < edges.size(); i++) {
            ...
            if(e -> type == 2) {
                if(i + 1 == edges.size()) 
                    result.push_back({1, e -> num});
            }else {
                result.push_back({0, e -> num});
                if(i - 1 >= 0)
