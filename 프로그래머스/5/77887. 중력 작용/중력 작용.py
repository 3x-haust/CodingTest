...
class treap_nd:
    ...
    def compran(self, nd):
        try:
            if self.ran > nd.ran: return True, True
...
class treap:
    ...
    def find_var(self, pos):
        ...
        while True:
            left_var = 0
            if now_nd.left: left_var = now_nd.left.w
            
            if pos < left_var: now_nd = now_nd.left
            elif pos == left_var: return now_nd.var
            else:
                pos -= left_var+1
                now_nd = now_nd.right
    ...
    def push(self, nd):
        if nd.ran > self.root.ran:
            ...
            while True:
                ...
                now_nd.sum += nd.sum
                if now_nd.right:
                    if now_nd.right.ran > nd.ran: now_nd = now_nd.right
                    else:
    ...
    def delete_nd(self, pos):
        ...
        while bp:
            ...
            if left_up:
                compS.sum += compL.sum
                compS.w += compL.w
                if isleft: anc.left = compS
                else: anc.right = compS
                anc = compS
                isleft = False
                compS = compS.right
            else:
                compL.sum += compS.sum
                compL.w += compS.w
                if isleft: anc.left = compL
                else: anc.right = compL
    ...
    def get_sum(self, pos):
        ...
        while True:
            left_w = 0
            if now_nd.left: left_w = now_nd.left.w
            if left_w > pos:
                now_sum -= now_nd.sum
                now_nd = now_nd.left
                now_sum += now_nd.sum
            elif left_w == pos:
                if now_nd.right: now_sum -= now_nd.right.sum
